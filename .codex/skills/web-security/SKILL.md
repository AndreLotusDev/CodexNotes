...
- Always scope authenticated data access to the current user unless the feature is explicitly public
- Pass the authenticated user's identity into database queries and mutations so records are filtered by ownership, not just by resource ID
- For collection reads, default to "this user's records" behavior; never return cross-user data unless the spec explicitly allows it
