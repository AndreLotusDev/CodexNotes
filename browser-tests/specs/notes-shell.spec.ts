...
test("authenticated notes shell guards private routes and exposes the shared header actions", async ({ page, appUrl }) => {
  await page.goto(`${appUrl}/notes`, { waitUntil: "domcontentloaded" });
  await expect(page).toHaveURL(`${appUrl}/login`);

  await page.goto(`${appUrl}/notes/new`, { waitUntil: "domcontentloaded" });
  await expect(page).toHaveURL(`${appUrl}/login`);

  await page.goto(`${appUrl}/notes/seed-demo-note`, { waitUntil: "domcontentloaded" });
  await expect(page).toHaveURL(`${appUrl}/login`);
...
test("notes list shows only the signed in user data, supports empty state, and resorts after edits", async ({
  ...
  await expect(page.getByRole("heading", { name: "Welcome to TinyNotes" })).toBeVisible();
});
